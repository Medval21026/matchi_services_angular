...
@Component({
  selector: 'app-modifier-client-modal',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, TranslatePipe],
  templateUrl: './modifier-client-modal.component.html',
  styleUrls: ['./modifier-client-modal.component.css']
})
export class ModifierClientModalComponent implements OnInit {
  @Input() isOpen = false;
  private _client?: ClientAbonneDTO;
  @Input()
  set client(value: ClientAbonneDTO | undefined) {
    this._client = value;
    if (value && this.clientForm) {
      this.loadClientData(value);
    }
  }
  get client(): ClientAbonneDTO | undefined {
    return this._client;
  }
  ...
  ngOnInit(): void {
    this.initForm();
    if (this._client) {
      this.loadClientData(this._client);
    }
  }
  ...
  private loadClientData(client: ClientAbonneDTO): void {
  ...
  onSubmit(): void {
    if (this.clientForm.invalid || !this._client?.id) {
    ...
    this.clientService.updateClient(this._client.id, client).subscribe({
      next: () => {
        this.isLoading = false;
        this.notificationService.showSuccess(this.translationService.translate('client.modifiedSuccess'));
        this.saved.emit();
        this.closeModal();
      },
  ...
  private extractErrorMessage(error: HttpErrorResponse): string {
    if (error.error instanceof ErrorEvent) {
      ...
      switch (error.status) {
        ...
        default:
          return `Erreur lors de la modification du client (Code: ${error.status}). ${backendMessage}`;
      }
    }
  }
